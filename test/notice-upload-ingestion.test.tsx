...
vi.mock("@/lib/ingestion/image-ocr-extractor", () => ({
  extractImageText,
}));
...
function createFile() {
  return new File(["notice"], "notice.pdf", {
    type: "application/pdf",
  });
}

function createImageFile() {
  return new File(["notice image"], "notice.png", {
    type: "image/png",
  });
}
...
function createIntelligenceResponse() {
  return Response.json({
    result: {
      notice: {
        title: "Exam Schedule",
        summary: "The exam schedule has been published.",
        issuer: {
...
describe("NoticeUploadIngestion", () => {
  ...
  it("displays extracted text and PDF source pages after a successful response", async () => {
    const fetchMock = vi.fn<FetchMock>(async (input) => {
      ...
      if (input === "/api/notices/intelligence") {
